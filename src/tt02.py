...
def run():
    ...
    # set the output path
    OUTPUT_VIDEO_PATH = "data/summarized/tt02/videos"
    OUTPUT_TXT_PATH = "data/summarized/tt02/txts"
    threshold = 400
    ...
    for video in videos_path:
        ...
        # tt01 algorithm
        selected_frames = tt02(features, threshold)
...
def test_1_sample():
    ...
    output_video = os.path.join("data/summarized/tt02/videos", f"summarized_{video_name}")
    to_video(selected_frames, frames, output_video, video_fps)

    output_txt = os.path.join("data/summarized/tt02/txts", f"{video_name}.txt")
    to_txt(selected_frames, output_txt, clip_sample_rate)
